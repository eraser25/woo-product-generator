...
def render_sidebar():
    ...
    settings = {
        ...
        "brand_code": st.sidebar.text_input(
            "Marka Kodu", value=saved_settings.get("brand_code", "KYX")
        )
        .strip()
        .upper()
        or "KYX",
        "product_type_code": st.sidebar.text_input(
            "Ürün Tipi Kodu", value=saved_settings.get("product_type_code", "GEN")
        )
        .strip()
        .upper()
        or "GEN",
    ...
    settings.update(
        {
            ...
            "ftp_use_tls": st.sidebar.checkbox(
                "FTPS kullan", value=saved_settings.get("ftp_use_tls", True)
            ),
            "ftp_host": st.sidebar.text_input(
                "FTP Host",
                value=saved_settings.get("ftp_host", ""),
                placeholder="ftp.siteadi.com",
            ),
            "ftp_port": st.sidebar.number_input(
                "FTP Port",
                min_value=1,
                max_value=65535,
                value=saved_settings.get("ftp_port", 21),
            ),
            "ftp_username": st.sidebar.text_input(
                "FTP Kullanıcı adı", value=saved_settings.get("ftp_username", "")
            ),
            "ftp_password": st.sidebar.text_input(
                "FTP Şifre",
                value=saved_settings.get("ftp_password", ""),
                type="password",
            ),
            "ftp_remote_dir": st.sidebar.text_input(
                "Remote klasör",
                value=saved_settings.get(
                    "ftp_remote_dir", "public_html/wp-content/uploads/products"
                ),
            ),
    ...
    remember_me = st.sidebar.checkbox(
        "💾 Bilgilerimi Hatırla (FTP & API)",
        value=saved_settings.get("remember_me", False),
    )
